...
| 画面 | 内容 |
| --- | --- |
| ホーム | 体重の ± 入力、今日のワークアウト、食事の残りカロリー/PFC（トレの推定消費カロリー付き）、よく食べるもの（お気に入りを優先表示）、AI コーチ（実データ＋要望を踏まえた食事・トレの提案、提案をそのまま登録） |
...
| 記録 | 週の振り返り（トレ日数・セット・消費 kcal / 平均 PFC と目標比 / 週平均体重、前週比）と AI の「来週の一言」（目標の変更提案つき）、体重推移（7日平均）、継続カレンダー、種目ごとの最大/合計回数の推移 |

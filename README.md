...
ブラウザで動く PWA からは Apple ヘルスケアや Health Connect のデータを直接読めないため、睡眠時間と歩数は前回値のプリセット＋ ± の手入力にしている。スマートウォッチや睡眠アプリが出す睡眠スコアも、睡眠の「詳細」からその場で手で写せる。将来、エクスポートしたファイルを取り込む形で連携を足す余地はある。

## 機能

| 画面 | 内容 |
| --- | --- |
| ホーム | 体重の ± 入力、睡眠・歩数の ± 入力（睡眠は「詳細」からスコアも記録可）、今日のワークアウト、食事の残りカロリー/PFC（トレの推定消費カロリー付き）、よく食べるもの（お気に入りを優先表示）、AI コーチ（実データ＋要望を踏まえた食事・トレの提案、提案をそのまま登録） |
| トレ | 「前回と同じで開始」/ 種目を選んで開始 / セットメニュー（種目 × セット数 × 目標回数・秒。手動作成、ワークアウトから保存、YouTube 動画から AI で取り込み）、セッション画面（✓ で完了 → 休憩タイマー自動開始、メニューの計画をプリセット、推定消費カロリー）、種目の追加は名前から AI で単位・部位・MET・フォームのコツを判定、履歴。種目・セットメニューはお気に入りを上位固定 |
| 食事 | リング/バーの目標比較、ワンタップ記録、ざっくり記録、AI 推定（写真 / 「牛丼の並とサラダ」のように食べたものを文章で伝える / 商品の栄養成分表示・原材料名を撮って登録）、食事セット、前日コピー。フード・食事セットはお気に入りを上位固定 |
| 記録 | 週の振り返り（トレ日数・セット・消費 kcal / 平均 PFC と目標比 / 週平均体重・睡眠・歩数、前週比）と AI の「来週の一言」（目標の変更提案つき）、体重・睡眠・歩数の推移（7日平均）、継続カレンダー、種目ごとの最大/合計回数の推移 |
| 設定 | プロフィールから目標を自動計算（Mifflin-St Jeor）/ AI に相談、消費カロリーを目標に加算する切替（既定 OFF）、休憩タイマー、AI プロバイダ、バックアップ |

### YouTube からのメニュー取り込み

ブラウザ単体では YouTube のページを読めない（CORS）ため、タイトルは oEmbed（noembed.com 経由）で取得し、説明欄・チャプターはユーザーが貼り付ける。Gemini は公開動画の URL をそのまま API に渡して動画本体を読める（`fileData.fileUri`）。Claude / ChatGPT はタイトルと貼り付けた文章から判定する。読み取った種目は既存種目と名寄せし、無いものは自作種目として作る。

### 消費カロリーの推定

`MET × 体重(kg) × 時間(h)` の目安。MET は種目の「動きのタイプ」ごとに自重運動の値（腕立て 3.8、スクワット 5.0、バーピー 8.0 など、`src/lib/calories.ts`）を割り当て、種目の編集で上書きできる。時間はセッションの開始〜終了（下限 1 セット 45 秒、上限 2 時間）。誤差は ±30% 程度なので常に「約」を付けて表示する。

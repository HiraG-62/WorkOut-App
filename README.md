...
ブラウザで動く PWA からは Apple ヘルスケアや Health Connect のデータを直接読めないため、睡眠時間と歩数は前回値のプリセット＋ ± の手入力にしている。将来、エクスポートしたファイルを取り込む形で連携を足す余地はある。

## 機能

| 画面 | 内容 |
| --- | --- |
| ホーム | 体重の ± 入力、睡眠・歩数の ± 入力、今日のワークアウト、食事の残りカロリー/PFC（トレの推定消費カロリー付き）、よく食べるもの（お気に入りを優先表示）、AI コーチ（実データ＋要望を踏まえた食事・トレの提案、提案をそのまま登録） |

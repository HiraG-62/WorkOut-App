...
| 画面 | 内容 |
| --- | --- |
| ホーム | 体重の ± 入力、今日のワークアウト、食事の残りカロリー/PFC（トレの推定消費カロリー付き）、よく食べるもの |
| トレ | 「前回と同じで開始」/ 種目を選んで開始 / セットメニュー（種目 × セット数 × 目標回数・秒。手動作成、ワークアウトから保存、YouTube 動画から AI で取り込み）、セッション画面（✓ で完了 → 休憩タイマー自動開始、メニューの計画をプリセット、推定消費カロリー）、種目の追加は名前から AI で単位・部位・MET・フォームのコツを判定、履歴 |
